                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="row mb-3">
                                <div class="col-md-3">
                                    <label for="filter_type" class="form-label">Filter Tipe</label>
                                    <select id="filter_type" class="form-select">
                                        <option value="">Semua Tipe</option>
                                        <option value="foto">Foto</option>
                                        <option value="video">Video</option>
                                        <option value="ebook">Ebook</option>
                                        <option value="ekoran">Ekoran</option>
                                    </select>
                                </div>
                            </div>
...
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/js/bootstrap.bundle.min.js"></script> --}}
    ...
    <script>
        const filterDropdown = document.getElementById("filter_type");

        // Ambil dari localStorage kalau ada
        const savedFilter = localStorage.getItem("filter_type");
        if (savedFilter) {
            filterDropdown.value = savedFilter;

            // trigger event change biar DataTable ikut reload
        ...
        $('#filter_type').on('change', function() {
            ...
            localStorage.setItem("filter_type", val);

            // bangun url dengan param type jika ada
            var url = '{{ route('galleries.index') }}';
            if (val !== '') {
                url += '?type=' + encodeURIComponent(val);
            }

            // reload DataTable dengan url baru
            ...
            buttons: [
                'pageLength', {
                    ...
                    action: function(e, dt, node, config) {
                        ...
                        var url = '{{ route('galleries.index') }}';
                        if (val !== '') url += '?type=' + encodeURIComponent(val);
        ...
        $(document).ready(function() {
            // table.ajax.url('{{ route('galleries.index') }}').load();
            var val = $('#filter_type').val() || '';
            var url = '{{ route('galleries.index') }}';
            if (val !== '') url += '?type=' + encodeURIComponent(val);
            table.ajax.url(url).load();
            ...
            $(document).on("click", "#addBtn", function() {
                ...
                // Reset modal sebelum menampilkannya
                resetGalleryModal();
        ...
        function resetGalleryModal() {
            ...
            // clear Cloudinary hidden inputs (public id, version, ext)
            $('#cover_public_id').val('');
            $('#cover_version').val('');
            $('#cover_ext').val('');
            $('#content_public_id').val('');
            $('#content_version').val('');
            $('#content_ext').val('');

            // clear inputs that mungkin dinamically dikasih value
            $('#new-cover_image_url').val('');
            $('#new-content_image_url').val('');

            // hide preview boxes & show upload buttons
            $('.show-cover-box').hide();
            $('.show-content-box').hide();
            $('#cover_image_url_btn').show();
            $('#content_image_url_btn').show();

            // kosongkan preview src
            $('#preview-cover').attr('src', '');
            $('#preview-content').attr('src', '');

            // remove ajax validation messages
            $('.ajax-invalid').remove();

            // enable submit button
            $('#submitGroupBtn').prop('disabled', false);

            // jika ada select2 / select, reset val (kalo pake select2, trigger change)
            if ($('#filter_tag').length) {
                $('#filter_tag').val('').trigger && $('#filter_tag').trigger('change');
            }
            if ($('#type').length) {
                $('#type').val('').trigger && $('#type').trigger('change');
            }
        }
    </script>
@endsection
